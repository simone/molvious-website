import { Team, BEVERLY, OLD, PANZAI, MAE, MERELLERS, TEAM9, CHELTRONES, ESQUILINO } from './Team';

export const PRIMA = BEVERLY;
export const SECONDA = OLD;
export const TERZA = PANZAI;
export const QUARTA = TEAM9;
export const QUINTA = MAE;
export const SESTA = MERELLERS;
export const SETTIMA = CHELTRONES;
export const OTTAVA = ESQUILINO;

export const SEMIFINALISTA1 = new Team("Semifinalista 1", "");
export const SEMIFINALISTA2 = new Team("Semifinalista 2", "");
export const SEMIFINALISTA3 = new Team("Semifinalista 3", "");
export const SEMIFINALISTA4 = new Team("Semifinalista 4", "");

export const FINALISTA1 = new Team("Finalista 1", "");
export const FINALISTA2 = new Team("Finalista 2", "");

export const TERZISTA3 = new Team("TerzoPosto 1", "");
export const TERZISTA4 = new Team("TerzoPosto 2", "");

export const FINALS = [
  PRIMA, SECONDA, TERZA, QUARTA,
  QUINTA, SESTA, SETTIMA, OTTAVA,
  SEMIFINALISTA1, SEMIFINALISTA2,
  SEMIFINALISTA3, SEMIFINALISTA4,
  FINALISTA1, FINALISTA2,
  TERZISTA3, TERZISTA4
]
